...
variable "cluster_vswitchs_map" {
  type = map(string)
  description = "A map of pairs {availability-zone = vswitch-suffix-number}. Each entry creates a vswitch. Minimum 1 pair.  (e.g. {\"cn-shanghai-b\" = 1} ) "
}
variable "management_vswitchs_map" {
  type = map(string)
  description = "A map of pairs {availability-zone = vswitch-suffix-number}. Each entry creates a vswitch. Minimum 1 pair.  (e.g. {\"cn-shanghai-b\" = 2} ) "
}
variable "private_vswitchs_map" {
  type = map(string)
  description = "A map of pairs {availability-zone = vswitch-suffix-number}. Each entry creates a vswitch. Minimum 1 pair.  (e.g. {\"cn-shanghai-b\" = 3} ) "
}
